...
pub async fn connect_procmux(hub: &AgentHub, socket_path: &str) {
    match ProcmuxProcessConnection::connect(socket_path).await {
        Ok(conn) => {
            ...
            match conn.list_agents().await {
                Ok(result) => {
                    ...
                    for agent_name in agents {
                        ...
                        // Fire and forget reconnect task
                        let hub_ref = hub.clone_ref();
                        let name = agent_name.clone();
                        let conn_ref = conn.clone();
                        tokio::spawn(async move {
                            reconnect_single(&hub_ref, &name, &conn_ref).await;
                        });
                    }
                }
                Err(e) => {
            ...
            *hub.process_conn.lock().await = Some(conn);
        }
        Err(e) => {
            warn!(
        ...
        Ok(result) => {
            ...
            match (hub.create_client)(name, resume_id.as_deref()).await {
                Ok(client) => {
                    ...
                    hub.scheduler.restore_slot(name).await;
                    drop(sessions);

                    let was_mid_task = cli_status == "running" && !cli_idle;
                    hub.callbacks.on_reconnect(name, was_mid_task).await;
                    info!("Reconnect complete for '{}'", name);
                }
                Err(e) => {
                    warn!("Failed to create client for reconnecting agent '{}': {:?}", name, e);
                    let mut sessions = hub.sessions.lock().await;
                    if let Some(session) = sessions.get_mut(name) {
                        session.reconnecting = false;
                    }
