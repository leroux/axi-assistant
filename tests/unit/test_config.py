...
from config import VALID_MODELS, get_model, set_model


class TestGetModel:
    def test_default_is_opus(self) -> None:
        with patch("config._load_config", return_value={}):
            assert get_model() == "opus"

    def test_returns_configured_model(self) -> None:
        with patch("config._load_config", return_value={"model": "sonnet"}):
            assert get_model() == "sonnet"


class TestSetModel:
    def test_valid_model(self) -> None:
        with patch("config._load_config", return_value={}), patch("config._save_config"):
