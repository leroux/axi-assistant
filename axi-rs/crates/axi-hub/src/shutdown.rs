...
impl ShutdownCoordinator {
    ...
    async fn get_busy_agents(&self, skip: Option<&str>) -> Vec<String> {
        let sessions = self.hub.sessions.lock().await;
        sessions
            .iter()
            .filter(|(name, session)| {
    ...
    async fn sleep_all(&self, skip: Option<&str>) {
        ...
        for name in names {
            lifecycle::sleep_agent(&self.hub, &name, true).await;
        }
    }

    async fn execute_exit(&self, skip_agent: Option<&str>) {
        // Start safety deadline thread
        ...
        self.hub.callbacks.send_goodbye().await;

        if !self.bridge_mode {
            self.sleep_all(skip_agent).await;
        }

        self.hub.callbacks.close_app().await;
        self.hub.callbacks.kill_process().await;
    }
